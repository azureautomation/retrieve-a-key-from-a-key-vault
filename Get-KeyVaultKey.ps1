<#
.SYNOPSIS
  This runbook will retrieve a key from the given key vault.
...
$key = Get-AzureKeyVaultKey -VaultName $keyVault[0].VaultName -Name $KeyName 
if ($key -eq $null)
{
	throw "Could not retrieve key $KeyName. Check whether a key with this name exists in this key vault."
}
